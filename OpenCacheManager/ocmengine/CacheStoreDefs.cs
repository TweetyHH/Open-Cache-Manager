...
namespace ocmengine
{
	...
	public partial class CacheStore
	{
		...
		const string LAST_FOUND = "SELECT MAX(date) from LOGS  WHERE (status='Found it' or status='find' or status='Attended') and cache='{0}' GROUP BY cache";
		const string LAST_FOUND_FILT = "SELECT MAX(date),cache from LOGS  WHERE (status='Found it' or status='find' or status='Attended') GROUP BY cache";
